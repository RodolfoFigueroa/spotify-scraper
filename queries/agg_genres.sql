COPY (
	...
	FROM (
		SELECT 
			...
			STDDEV_SAMP(duration_ms) AS duration_ms_std,
			STDDEV_SAMP(danceability) AS danceability_std,
			STDDEV_SAMP(energy) AS energy_std,
			STDDEV_SAMP(loudness) AS loudness_std,
			STDDEV_SAMP(speechiness) AS speechiness_std,
			STDDEV_SAMP(acousticness) AS acousticness_std,
			STDDEV_SAMP(instrumentalness) AS instrumentalness_std,
			STDDEV_SAMP(liveness) AS liveness_std,
			STDDEV_SAMP(valence) AS valence_std,
			STDDEV_SAMP(tempo) AS tempo_std,
		...
		FROM songs, artists, artists_songs, artists_genres
		WHERE 
			artists_songs.song = songs.id AND
			artists_songs.artist = artists.id AND 
			artists_genres.artist = artists.id
		GROUP BY artists_genres.genre
	) AS t1
) TO '/media/postgres2/genres_aggregated.csv' CSV HEADER