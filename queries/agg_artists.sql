COPY (
	...
	FROM (
		...
		FROM artists, songs, artists_songs
		WHERE artists.id = artists_songs.artist AND songs.id = artists_songs.song
		GROUP BY artists.id
	) AS t1
) TO '/media/postgres2/artists_aggregated.csv' CSV HEADER